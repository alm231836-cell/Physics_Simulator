...
public class SpeedVelocityView extends BorderPane {
    ...
    private final Slider timeSlider = new Slider(1, 20, 5);
    private final Label timeLabel = new Label("5.0 s");
    private final Label speedLabel = new Label("0.00 m/s");
    private final Label velocityLabel = new Label("0.00 m/s");
    ...
    private double velocityMetersPerSecond = 0;
    ...
    private static final double ARROW_LENGTH = 42;

    public SpeedVelocityView() {
        ...
        timeSlider.valueProperty().addListener((obs, old, value) -> {
            timeLabel.setText(String.format("%.1f s", value.doubleValue()));
            updateCalculations();
            drawCanvas();
        });
        ...
        VBox controls = new VBox(16,
                ...
                statBlock("Trip Time", timeLabel, "Time to complete the drawn path"),
                timeSlider,
                statBlock("Average Speed", speedLabel, "Orange: Path length ÷ time (scalar)"),
                statBlock("Average Velocity", velocityLabel, "Green arrow: Displacement ÷ time (vector magnitude)")
        );
    ...
    private boolean isOnStart(MouseEvent event) {
        if (startX == -1) {
            return false;
        }
    ...
    private boolean isOnEnd(MouseEvent event) {
        if (endX == -1 || isDrawingPath) {
            return false;
        }
        double dx = event.getX() - endX;
        double dy = event.getY() - endY;
        return Math.sqrt(dx * dx + dy * dy) <= POINT_RADIUS + 5;
    }
    ...
    private void clearAll() {
        ...
        speedLabel.setText("0.00 m/s");
        velocityLabel.setText("0.00 m/s");
        canvas.setCursor(Cursor.CROSSHAIR);
        drawCanvas();
    }
    ...
    private void updateCalculations() {
        double tripTime = timeSlider.getValue();
        boolean includeLiveEnd = isDrawingPath;

        speedMetersPerSecond = SpeedVelocityModel.calculateAverageSpeedMetersPerSecond(
                pathX, pathY, endX, endY, includeLiveEnd, tripTime);
        velocityMetersPerSecond = SpeedVelocityModel.calculateAverageVelocityMetersPerSecond(
                startX, startY, endX, endY, tripTime);

        speedLabel.setText(String.format("%.2f m/s", speedMetersPerSecond));
        velocityLabel.setText(String.format("%.2f m/s", velocityMetersPerSecond));
    }
    ...
    private void drawCanvas() {
        ...
        if (isDrawingPath) {
            ...
            gc.fillText("Click and drag from start to end to compare speed & velocity", 20, 30);
        } else {
        ...
        if (startX != -1 && endX != -1) {
            ...
            drawVelocityArrow(startX, startY, endX, endY);
        }
        ...
        if (startX != -1 && endX != -1 && (speedMetersPerSecond > 0 || velocityMetersPerSecond > 0)) {
            double labelX = (startX + endX) / 2 + 12;
            double labelY = (startY + endY) / 2 - 10;
            gc.setFill(Color.web("#1e293b"));
            gc.setFont(javafx.scene.text.Font.font(13));
            gc.fillText(String.format("speed = %.2f m/s", speedMetersPerSecond), labelX, labelY);
            gc.fillText(String.format("|v| = %.2f m/s", velocityMetersPerSecond), labelX, labelY + 18);
        }
    ...
    private void drawVelocityArrow(double x1, double y1, double x2, double y2) {
        double dx = x2 - x1;
        double dy = y2 - y1;
        ...
        double midX = (x1 + x2) / 2;
        double midY = (y1 + y2) / 2;
        double arrowLen = Math.min(ARROW_LENGTH, length * 0.45);
        double tipX = midX + nx * arrowLen / 2;
        double tipY = midY + ny * arrowLen / 2;
        double tailX = midX - nx * arrowLen / 2;
        double tailY = midY - ny * arrowLen / 2;
        ...
        gc.strokeLine(tipX, tipY,
                tipX - wing * Math.cos(angle - Math.PI / 6),
                tipY - wing * Math.sin(angle - Math.PI / 6));
        gc.strokeLine(tipX, tipY,
                tipX - wing * Math.cos(angle + Math.PI / 6),
                tipY - wing * Math.sin(angle + Math.PI / 6));
    }
